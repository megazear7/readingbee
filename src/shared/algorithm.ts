...
import { pictureFor, trackPictureSeen } from "./letter-pictures.js";
import {
  emptyTextStat,
  LETTERS_MAX_LEVEL,
  MAX_LEVEL,
...
export const recordAndPickNext = (
  ...
  const showedPicture = Boolean(pictureFor(text, profile, at));
  let updated = applyResult(profile, text, result, rng, at, corpus);
  if (showedPicture) {
    updated = trackPictureSeen(updated, text.id, profile, at);
  }
